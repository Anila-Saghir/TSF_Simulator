...
with tabs[3]:
    ...
    if not st.session_state.get("frame"):
        ...
        # --- Plan generation ---
        st.markdown("### Select Days to Generate plan")
        c1, c2 = st.columns([1, 1])
        with c1:
            num_days = st.number_input("Please Select Number of days for correction", min_value=1, max_value=365, value=5, step=1)
        with c2:
        ...
        if generate:
            ...
            # Start from user Day-1 lengths
            start_lengths = np.minimum(np.maximum(init_lengths.copy(), minL), maxL)

            # Interpolate lengths from Day-1 → target
        ...
        if plan_obj:
            days = plan_obj["days"]
            st.markdown(f"### Strut Adjustment Plan for — {days} days")
            ...
            st.markdown("Visualise the Bone Correction Day by Day:")
            selected_day = st.number_input("Pick day", min_value=1, max_value=days, value=1, step=1)

            if st.button("Show Visualization") or st.session_state.get("_tmp_selected"):
                ...
                # Correct direction: start from deformed and move toward corrected
                R_interp = rot_matrix_from_euler(-roll*(1-t), -pitch*(1-t), -yaw*(1-t))
                T_interp = np.array([-X*(1-t), -Y*(1-t), -Z*(1-t)])
                top_pts_day = transform_points(plan_obj["top_attach"], R_interp, T_interp)

                bottom = plan_obj["bottom_attach"]
                ...
                top_ring_moved = rigid_transform(plan_obj["top_attach"], top_pts_day, top_ring)
                ...
                def add_bone(fig, p_base, p_tip, base_r, tip_r, color):
                    x, y, z, I, J, K = pencil_mesh(p_base, p_tip, base_r, tip_r)
                    if x is not None:
                        fig.add_trace(go.Mesh3d(x=x, y=y, z=z, i=I, j=J, k=K, color=color, opacity=0.85))

                distal_centroid = bottom.mean(axis=0)
                distal_tip = distal_centroid + np.array([0, 0, 1]) * bone_length
                add_bone(fig, distal_centroid, distal_tip, 14, 6, '#d7c5a3')

                proximal_centroid = top_pts_day.mean(axis=0)
                proximal_tip = proximal_centroid + np.array([0, 0, -1]) * bone_length
                add_bone(fig, proximal_centroid, proximal_tip, 14, 6, '#c2a177')

                fig.update_layout(
                    scene=dict(aspectmode='auto'),
                    width=1000, height=700,
                    title=f"Day {selected+1} Bone Alignment"
                )
                st.plotly_chart(fig, use_container_width=True)
